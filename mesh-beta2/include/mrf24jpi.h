 ...
 * 
 * Ported to C by James Hoyland, 2018
 * 
 */
...
/*
 * The MRF unit is accessed through the microcontroller's SPI interface.
 * To do so you read or write to the various registers using these functions
 * Some registers have 8-bit addresses some have 16 bit addresses. The read functions
 * return a single byte of data (the value of the register specified). 
 * The write functions send a single byte to the register
 * */
...
/*
 * Set / get the PANID for the network. This does not interact with network itself
 * only telling the module which value to use for this ID. 
 * */
...
/**
 * This is the main business - this is the function that actually transmits data to the network
 * */
